...
import { ACRYL_ASSET } from './transactionData';
...
describe('Utils test', () => {
    ...
    it('getCoins', () => {
        expect(getCoins(new Money('100', ACRYL_ASSET))).toBe('100');
        expect(typeof getCoins(new Money('100', ACRYL_ASSET))).toBe('string');
        expect(getCoins(new BigNumber('100'))).toBe('100');
        expect(typeof getCoins(new BigNumber('100'))).toBe('string');
    });

    it('getCoins', () => {
        expect(getAssetId(new Money('100', ACRYL_ASSET))).toBe('ACRYL');
    });
