...
import { IAliasTransaction } from '@acryl/ts-types';
...
export const alias = factory<IAcrylGuiAlias, IAliasTransaction<string>>({
    ...getDefaultTransform(),
    alias: pipe(
        prop('alias'),
        validate(
...
export interface IAcrylGuiAlias extends IDefaultGuiTx<typeof TYPES.ALIAS> {
    alias: string;
}
